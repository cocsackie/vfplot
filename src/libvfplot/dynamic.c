/*
  ...
  J.J.Green 2007
  $Id: dynamic.c,v 1.42 2008/01/10 00:30:45 jjg Exp $
*/
...
/* expand p so it fits n particles (and put its new size in na) */

static int ensure_alloc(int n, particle_t **pp,int *na)
{
  particle_t *p;

  if (n <= *na) return 0;

  if ((p = realloc(*pp,n*sizeof(particle_t))) == NULL) return 1;
...
{
  s->mass   = 1.0;
  s->charge = sinspline(t,CONTAIN_T0,10.0,CONTAIN_T1,1.0);
  s->rd     = 0.0;
  s->rt     = 0.0;
  s->dmax   = 0;
}
...
extern int dynamic(dynamic_opt_t opt,int* nA,arrow_t** pA,int* nN,nbs_t** pN)
{
  int i,err;
  ...
  /*
    n1 number of fence ellipses
    n2 number of free ellipses 
    na number of ellipses allocated
  */
  ...
  */
  ...
  int ni = M_PI*w*h/(sqrt(12)*opt.area);
  ...
  if (ni<1)
    {
      fprintf(stderr,"bad initial number of arraow %i\n",ni);
      return ERROR_NODATA;
    }
  ...
  */
  ...
  /* transfer fence ellipses to paticle array */

  for (i=0 ; i<n1 ; i++)
    {
      ellipse_t E = opt.pE[i];
  ...
  /* generate an interior paticle set on a regular grid */
	  ...
	  if (opt.v.place.adaptive.animate)
	    {
	      ...
	      if (n2 > *nA)
		{
		  arrow_t* A = realloc(*pA,n2*sizeof(arrow_t));
